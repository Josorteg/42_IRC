...
/*   Updated: 2024/05/30 00:02:41 by mmoramov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::_botServer(Client &client, std::vector<std::string> parsedCommand)
{
	...
	*/
	...
	if (parsedCommand[1] == "ON")
	{
		...
		message = ":BOT!~" + client.getHostname() + " PRIVMSG " + channel.getName() + " " + ":Hello, bad words are forbidden from now on. If you use a bad word three times, I will kick you.";
		_sendMessage(channel, 0, message);
	}
...
void Server::_botchecker(Client &client, Channel &channel, std::string &message)
{
	...
	else if (channel.getBadWordCounter(client) == 2)
	{
		...
		reply += ":This is the second time you have used a bad word. If you do it again, you will be kicked from the channel.";
		_sendMessage(client, reply);
	}
	else if (channel.getBadWordCounter(client) == 1)
	{
		reply = ":BOT!" + client.getHostname() + " PRIVMSG " + channel.getName() + " ";
		reply += ":You are using bad words " + client.getNickname() + ". It's the first time you have used a bad word. It is forbidden.";
		_sendMessage(client, reply);
	}
}
